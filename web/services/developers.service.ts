...
export const developersService = {
  ...
  async createWebhook(url: string, events: string[]): Promise<WebhookEndpoint> {
    if (API_CONFIG.MOCK_MODE) {
      await delay(800);
      return {
        ...
        status: 'active',
        createdAt: new Date().toISOString(),
        signingSecret: `whsec_${Math.random().toString(36).substring(7)}`
      };
    }
    return apiClient.post<WebhookEndpoint>(API_CONFIG.ENDPOINTS.DEVELOPERS.WEBHOOKS, { url, events });
  },
  ...
  async deleteWebhook(id: string): Promise<boolean> {
    ...
    await apiClient.delete(`${API_CONFIG.ENDPOINTS.DEVELOPERS.WEBHOOKS}/${id}`);
    return true;
  },
