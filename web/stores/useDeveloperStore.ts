...
export const useDeveloperStore = create<DeveloperState>((set, get) => ({
  ...
  createWebhook: async (url: string, events: string[]) => {
    set({ isLoading: true });
    try {
      const newHook = await developersService.createWebhook(url, events);
      set((state) => ({ 
        webhooks: [...state.webhooks, newHook],
        isLoading: false 
      }));
    } catch (error) {
      set({ error: 'Failed to create webhook', isLoading: false });
    }
  },
  ...
  deleteWebhook: async (id: string) => {
    ...
    try {
      ...
      set({ error: 'Failed to delete webhook', isLoading: false });
    }
  },
