  ...
  return (
    <div className="relative bg-gradient-to-br from-slate-50 to-white rounded-2xl border border-gray-200 shadow-xl overflow-hidden" style={{ aspectRatio: "4/3" }}>
      <svg viewBox="0 0 100 100" className="w-full h-full p-2">
        ...
        {/* Center hub */}
        <circle cx={50} cy={50} r={12} fill="white" stroke="#ec4899" strokeWidth="1" />
        <text x={50} y={50} textAnchor="middle" dominantBaseline="central" fontSize="7">⚡</text>
        <text x={50} y={67} textAnchor="middle" fontSize="3" fontWeight="bold" fill="#ec4899">MediBridgeX</text>
        {/* Entity nodes */}
        {entities.map((e, i) => (
          <g key={i}>
            <motion.circle
            ...
            />
            <text x={e.x} y={e.y} textAnchor="middle" dominantBaseline="central" fontSize="5.5">{e.emoji}</text>
            <text x={e.x} y={e.label === "Hospital" ? e.y - 11.5 : e.y + 11.5} textAnchor="middle" fontSize="2.8" fill="#64748b" fontWeight="600">{e.label}</text>
          </g>
        ))}
      </svg>
    </div>
